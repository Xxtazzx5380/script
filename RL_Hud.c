class RL_Hud : SCR_InfoDisplayExtended
{
	...
	protected ref map<string, ref Widget> m_activeNotifications = new map<string, ref Widget>();
	...
	void AddNotification(string content, string title = "", int duration = 5)
	{
		...
		string notiId = RL_Utils.GenerateRandomUid();
		m_activeNotifications.Insert(notiId, notiWidget);
		GetGame().GetCallqueue().CallLater(DeleteNotification, duration * 1000, false, notiId);
	}

	void DeleteNotification(string notiId)
	{
		int count = m_activeNotifications.Count();
		if (count < 1 || !m_activeNotifications.Get(notiId))
			return;
		m_notificationContainer.RemoveChild(m_activeNotifications.Get(notiId));
		m_activeNotifications.Remove(notiId);
	}
	...
	void AddIdentity(Widget identityWidget, int duration = 5)
	{
		...
		m_activeNotifications.Insert(notiId, identityWidget);
		GetGame().GetCallqueue().CallLater(DeleteNotification, duration * 1000, false, notiId);
	}
